  ...
  const validateSubmition = () => {
    ...
    if (fname && lname && username && email && password) {
      ...
      Swal.fire({
        icon: "error",
        title: "Invalid input",
        text: "Your inputs are wrong or invalid!",
      });
    }
  };
  ...
  return (
    <section className={"flex justify-center items-center p-5"}>
      <div className={"w-fit p-10 border shadow-xl rounded-xl"}>
        <img
          src="https://logodix.com/logo/1597040.png"
          title="logo"
          alt="logo"
        ...
        <div className={"text-center mt-5"}>
          <button className={"main-btn"} onClick={validateSubmition}>
            Sign Up
          </button>
        </div>
        ...
        <div className={"text-center mt-5"}>
          Do have an account?{" "}
          <Link to={"/signin"}>
            <span className={"text-blue-600 underline"}>Sign in now</span>
          </Link>
        </div>
      </div>
    </section>
